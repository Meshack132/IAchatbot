...
if __name__ == "__main__":
    ...
    st.sidebar.markdown("<div class='made_by'>Made by MESH132🔋</div>", unsafe_allow_html=True)

    ### POSTGRES DB OBJECTS VIEWER ###
    st.markdown(made_by_sdw, unsafe_allow_html=True)
    st.sidebar.title("🔍 Postgres DB Objects Viewer")

    # Check if sidebar_data is not empty
    if sidebar_data:
        # Dropdown for schema selection with a default value
        selected_schema = st.sidebar.selectbox("📂 Select a schema", list(sidebar_data.keys()), index=0)

        # Ensure selected_schema is valid
        if selected_schema:
            # Dropdown for table selection based on chosen Schema
            selected_table = st.sidebar.selectbox("📜 Select a table", list(sidebar_data[selected_schema].keys()), index=0)
            ...
            st.sidebar.error("No schema selected.")
    else:
        st.sidebar.error("No schema data available.")
